...
pub fn unified_change_system(
    ...
    // All change detection queries in one place - simplified to avoid conflicts
    ...
    removed_movements: RemovedComponents<Movement>, 
    removed_healths: RemovedComponents<Health>,
    removed_names: RemovedComponents<Name>,
) {
    let system_start = Instant::now();
    let correlation_id = LoggingSystem::generate_correlation_id();
    
    // Count changes for performance tracking
    ...
    if total_changes > 0 {
        debug!(
            ...
            "Processing component changes"
        );
    }
    // === MONITORING (count changes for statistics) ===
    ...
    monitor.record_removals::<Position>(removed_positions.len());
    monitor.record_removals::<Movement>(removed_movements.len());
    monitor.record_removals::<Health>(removed_healths.len());
    monitor.record_removals::<Name>(removed_names.len());
    ...
    if game_time.tick == 0 && game_time.turn > 1 {
        ...
        for (entity, mut movement, name) in changed_movements.iter_mut() {
            ...
            debug!(
                ...
                restored_movement = movement.current(),
    ...
    #[cfg(debug_assertions)]
    {
        ...
        for (entity, health, name) in changed_healths.iter() {
            ...
            tracing::debug!("Entity {:?} '{}' health: {:.1}/{:.1}", 
                           entity, name_str, health.current(), health.maximum());
        }
        ...
        for (entity, movement, name) in changed_movements.iter() {
            ...
            debug!(
                ...
                entity = ?entity,
                name = name_str,
                current_movement = movement.points(),
                max_movement = movement.max_points(),
...
mod tests {
    ...
    fn test_recording_changes() {
        ...
        let stats = monitor.get_stats::<Position>().unwrap();
