...
/// Bundle for tiles (terrain entities)
#[derive(Bundle, Debug, Clone)]
...
impl EntityFactory {
    ...
    pub fn create_unit(
        ...
        let entity = commands.spawn(UnitBundle {
            position: Position::new_unchecked(hex_pos.x, hex_pos.y),
            movement: Movement::new(get_unit_movement(unit_type), get_unit_max_moves(unit_type), get_unit_movement_type(unit_type))?,
            health: Health::new(get_unit_health(unit_type))?,
            renderable: Renderable::new(format!("unit_{}", unit_type)),
    ...
    pub fn create_city(
        ...
        let entity = commands.spawn(LivingEntityBundle {
            ...
            renderable: Renderable::new("city".to_string()),
...
/// Get max movement points for different unit types
fn get_unit_max_moves(unit_type: &str) -> u32 {
    match unit_type.to_lowercase().as_str() {
        "scout" | "cavalry" => 3,
        "infantry" | "archer" => 2,
        "siege" => 1,
        "naval" => 4,
        _ => 2, // Default max moves
    }
}

/// Get movement type for different unit types
fn get_unit_movement_type(unit_type: &str) -> crate::ecs::components::core::MovementType {
    use crate::ecs::components::core::MovementType;
    match unit_type.to_lowercase().as_str() {
        "naval" => MovementType::Naval,
        "scout" | "cavalry" | "infantry" | "archer" | "siege" => MovementType::Land,
        _ => MovementType::Land, // Default to land movement
    }
}
