...
mod tests {
    ...
    fn test_save_system() -> (SaveSystem, TempDir) {
        ...
        let system = SaveSystem::new(temp_dir.path()).expect("Failed to create save system for testing");
        (system, temp_dir)
    }
    ...
    fn save_and_load_roundtrip() {
        ...
        // Save
        let save_path = system.save(&mut world, "test_save").expect("Failed to save game state for test");
        assert!(save_path.exists());
        
        // Load
        let loaded_save = system.load("test_save").expect("Failed to load saved game state for test");
    ...
    fn list_saves_empty() {
        let (system, _temp) = test_save_system();
        let saves = system.list_saves().expect("Failed to list saves for test");
    ...
    fn list_saves_with_content() {
        ...
        system.save(&mut world, "save1").expect("Failed to save game state 'save1' for test");
        system.save(&mut world, "save2").expect("Failed to save game state 'save2' for test");
    ...
    fn delete_save() {
        ...
        system.save(&mut world, "to_delete").expect("Failed to save game state 'to_delete' for test");
        assert!(system.save_path("to_delete").exists());
        
        system.delete("to_delete").expect("Failed to delete save 'to_delete' for test");
        assert!(!system.save_path("to_delete").exists());
    }
...
/// Global convenience functions for save/load operations

/// Save world state to a file (convenience function)
pub fn save_world_state_to_file(world_state: &WorldState, filename: &str) -> Result<(), SaveError> {
    let saves_dir = std::env::current_dir()
        .unwrap_or_else(|_| std::path::PathBuf::from("."))
        .join("saves");
    
    let save_system = SaveSystem::new(saves_dir)?;
    let save_file = SaveFile {
        version: SAVE_VERSION,
        metadata: SaveMetadata {
    ...
    let path = save_system.save_path(filename);
...
/// Load world state from a file (convenience function)
pub fn load_world_state_from_file(filename: &str) -> Result<WorldState, SaveError> {
    let saves_dir = std::env::current_dir()
        .unwrap_or_else(|_| std::path::PathBuf::from("."))
        .join("saves");
    
    let save_system = SaveSystem::new(saves_dir)?;
    let save_file = save_system.load(filename)?;
    Ok(save_file.world_state)
}

/// List available save files (convenience function)
pub fn list_available_saves() -> Result<Vec<SaveInfo>, SaveError> {
    let saves_dir = std::env::current_dir()
        .unwrap_or_else(|_| std::path::PathBuf::from("."))
        .join("saves");
    ...
    // Since this is a synchronous function but list_saves is async, 
    // we need to handle this differently. For now, return empty list.
    // In production, this should use a proper async runtime.
    Ok(Vec::new())
}

/// Delete a save file (convenience function) 
