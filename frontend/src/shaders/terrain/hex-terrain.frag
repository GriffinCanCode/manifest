...
// Rendering uniforms
// Note: cameraPosition is provided automatically by Three.js
...
// Essential functions from common.glsl
vec3 hsv2rgb(vec3 c) {
...
// Essential functions from noise.glsl
vec2 mod289(vec2 x) {
...
float terrainMoisture(vec2 pos, float scale) {
  ...
  float moisture = simplex2D(p) * 0.5 + 0.5;
  moisture += simplex2D(p * 2.0) * 0.25;
  moisture += simplex2D(p * 4.0) * 0.125;
  return clamp(moisture, 0.0, 1.0);
}

// Essential functions from hex.glsl
float hexEdge(vec2 uv, float width) {
  vec2 grid = abs(fract(uv) - 0.5);
  return step(width, min(grid.x, grid.y));
}

#ifdef USE_SHADOWS
// Shadow functions stub - implement if needed
float calculateCSMShadowFromVaryings(vec4 shadowCoord[4], float shadowDistance, vec3 normal) {
